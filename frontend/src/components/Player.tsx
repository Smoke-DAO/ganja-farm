...
import { can_harvest, GardenVectorOutput } from "../api/farmContract";
...
export default function Player({
  ...
  async function checkTiles() {
    ...
    if (isInGarden) {
      ...
      try {
        const canHarvest = await can_harvest(distances[0].i);
        ...
        console.log("Error checking harvest status:", err);
      }
    } else if (isAtMarket) {
      setModal("market");
    }
